...
export async function generateWithProvider(env, body) {
  const prompt = [
    ...
    '必须逐日分析 selectedDays 的日期顺序和间隔：前后训练日只隔 1 天时降低连续疲劳；间隔 2 到 3 天时可以稳步推进；间隔 4 天以上时先找回状态再加量。',
    '必须参考 diary 中最近实际训练成果：完成动作、跳过内容、训练时长、热量和心率。如果最近完成度低或跳过多，下一次计划降低容量或换恢复型重点；如果完成度高且间隔合理，可以适度增加刺激。',
    '每个计划必须填写 intensityLevel、intensityReason、trainingBenefit。intensityReason 要说明当天日期、与前一次训练的间隔、前几天训练重点、实际训练成果和为什么按这个强度安排；trainingBenefit 要说明这些训练可以加强哪些肌群、能力或体态目标，并说明它和前几天训练如何衔接。',
